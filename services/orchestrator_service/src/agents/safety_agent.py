...
class SafetyAgentSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="ORCHESTRATOR_SAFETY_AGENT_", env_file=".env", extra="ignore"
    )
...
class SafetyCheckResult:
    ...
    def from_dict(cls, data: dict[str, Any]) -> SafetyCheckResult:
        return cls(
            ...
            protective_factors=[
                ProtectiveFactorDTO.from_dict(pf) for pf in data.get("protective_factors", [])
            ],
            ...
            for attempt in range(self._settings.max_retries + 1):
                try:
                    response = await client.post(
                        f"{self._base_url}/api/v1/safety/check", json=request.to_dict()
                    )
...
class SafetyAgent:
    ...
    async def _perform_safety_check(
        self, user_id: str, session_id: str, message: str, messages: list[dict[str, Any]]
    ) -> SafetyCheckResult:
        crisis_indicators = [
            "suicide",
            "kill myself",
    ...
    def _build_state_update(
        self, result: SafetyCheckResult, existing_flags: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        monitoring = (
            ...
            if result.crisis_level in (CrisisLevel.HIGH, CrisisLevel.CRITICAL)
                ...
                or (result.risk_factors and any(rf.severity > 0.5 for rf in result.risk_factors))
                else "standard"
            )
        )
        esc_reason = None
        if result.requires_escalation:
            ...
            esc_reason = (
                f"High severity risk factors: {', '.join(high_sev)}"
                if high_sev
                else (
                    f"Multiple risk factors detected ({len(result.risk_factors)} factors)"
                    if result.risk_factors
                    else f"Crisis level {result.crisis_level.value} detected"
                )
            )
        ...
        if crisis_detected:
            resources_text = (
                ...
                + "".join(
                    f"- **{r.get('name', 'Crisis Line')}**: {r.get('contact', '')}\n"
    ...
    def _build_fallback_response(
        self, message: str, existing_flags: dict[str, Any]
    ) -> dict[str, Any]:
        crisis_keywords = [
            "suicide",
            "kill myself",
