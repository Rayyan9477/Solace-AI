...
from services.orchestrator_service.src.langgraph.router import (
    Router,
    RouterSettings,
    RoutingStrategy,
    RouteDecision,
...
class TestRouterSettings:
    ...
    def test_custom_settings(self):
        ...
        assert settings.max_parallel_agents == 2


class TestIntentAnalyzer:
    """Tests for IntentAnalyzer."""

    def test_analyze_crisis_message(self):
        """Test crisis message analysis."""
        analyzer = IntentAnalyzer()
        result = analyzer.analyze("I want to kill myself")
        assert result.intent == IntentType.CRISIS_DISCLOSURE
        assert result.confidence >= 0.9
        assert len(result.risk_indicators) > 0

    def test_analyze_emotional_message(self):
        """Test emotional message analysis."""
        analyzer = IntentAnalyzer()
        result = analyzer.analyze("I feel so depressed and anxious today")
        assert result.intent == IntentType.EMOTIONAL_SUPPORT
        assert result.confidence >= 0.6
        assert len(result.emotional_indicators) >= 2

    def test_analyze_symptom_message(self):
        """Test symptom discussion analysis."""
        analyzer = IntentAnalyzer()
        result = analyzer.analyze("I have trouble with sleep and my appetite has changed")
        assert result.intent == IntentType.SYMPTOM_DISCUSSION
        assert len(result.topic_signals) > 0

    def test_analyze_treatment_message(self):
        """Test treatment inquiry analysis."""
        analyzer = IntentAnalyzer()
        result = analyzer.analyze("What therapy techniques can help me?")
        assert result.intent == IntentType.TREATMENT_INQUIRY

    def test_analyze_general_message(self):
        """Test general chat analysis."""
        analyzer = IntentAnalyzer()
...
class TestAgentSelector:
    ...
    def test_select_crisis_agents(self):
        """Test agent selection for crisis."""
        settings = RouterSettings()
        selector = AgentSelector(settings)
        analysis = IntentAnalysis(
            intent=IntentType.CRISIS_DISCLOSURE,
            confidence=0.95,
            matched_patterns=["kill myself"],
            emotional_indicators=[],
            risk_indicators=["kill myself"],
            topic_signals=[],
        )
        decision = selector.select(analysis, {}, False)
        assert AgentType.SAFETY in decision.primary_agents
        assert decision.priority == "critical"

    def test_select_emotional_agents(self):
        """Test agent selection for emotional support."""
        settings = RouterSettings()
        selector = AgentSelector(settings)
        analysis = IntentAnalysis(
            intent=IntentType.EMOTIONAL_SUPPORT,
            confidence=0.8,
            matched_patterns=["depressed"],
            emotional_indicators=["depressed"],
            risk_indicators=[],
            topic_signals=[],
        )
        decision = selector.select(analysis, {}, False)
        assert AgentType.THERAPY in decision.primary_agents
        assert decision.priority == "high"

    def test_select_with_active_crisis_flags(self):
        """Test selection with active crisis safety flags."""
        settings = RouterSettings()
        selector = AgentSelector(settings)
        analysis = IntentAnalysis(
...
class TestRouteDecision:
    ...
    def test_route_decision_to_dict(self):
        """Test route decision serialization."""
        decision = RouteDecision(
            primary_agents=[AgentType.THERAPY],
            secondary_agents=[AgentType.PERSONALITY],
            strategy=RoutingStrategy.PARALLEL,
            reasoning="Test routing",
            priority="normal",
        )
        result = decision.to_dict()
        assert result["primary_agents"] == ["therapy"]
        assert result["secondary_agents"] == ["personality"]
        assert result["strategy"] == "parallel"

    def test_all_agents(self):
        """Test all_agents method."""
        decision = RouteDecision(
            primary_agents=[AgentType.THERAPY],
            secondary_agents=[AgentType.PERSONALITY],
            strategy=RoutingStrategy.PARALLEL,
        ...
        all_agents = decision.all_agents()
        assert len(all_agents) == 2
        assert AgentType.THERAPY in all_agents


class TestRouter:
    """Tests for Router."""

    def test_router_process(self):
        ...
        assert result["processing_phase"] == ProcessingPhase.AGENT_ROUTING.value

    def test_router_crisis_detection(self):
        """Test router crisis detection."""
        router = Router()
        state = create_initial_state(
            user_id="user-123",
            session_id="session-456",
        ...
        result = router.route(state)
        assert result["intent"] == IntentType.CRISIS_DISCLOSURE.value
