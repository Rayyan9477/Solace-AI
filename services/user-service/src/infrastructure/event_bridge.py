"""
Solace-AI User Service - Kafka Event Bridge.
Bridges user lifecycle events to Kafka for inter-service messaging.
"""
...
from ..events import DomainEvent, to_kafka_event
...
class KafkaEventBridge:
    ...
    def __init__(
        ...
        self._started = False
        logger.info("user_kafka_bridge_initialized")
    ...
    async def stop(self) -> None:
        ...
        if self._publisher and self._started:
            ...
            self._started = False
            logger.info("user_kafka_bridge_stopped")

    async def bridge_event(self, event: DomainEvent) -> bool:
        """Convert and publish a user event to Kafka.
