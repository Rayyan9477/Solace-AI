"""
Solace-AI Notification Service - Kafka Event Bridge.
Bridges notification delivery events to Kafka for analytics tracking.
"""
...
from ..events import NotificationEvent, to_kafka_event
...
class KafkaEventBridge:
    """Bridges notification events to Kafka."""
    ...
    def __init__(
        ...
        self._started = False
        logger.info("notification_kafka_bridge_initialized")
    ...
    async def bridge_event(self, event: NotificationEvent) -> bool:
        """Convert and publish a notification event to Kafka.
        ...
        try:
            ...
            logger.error(
                "notification_kafka_bridge_error",
                event_type=event.event_type.value,
                error=str(e),
            )
            return False
