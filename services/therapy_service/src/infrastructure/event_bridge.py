...
Bridges therapy session and intervention events to Kafka for inter-service messaging.

Delegates conversion logic to events.to_kafka_event() to avoid duplication.
"""
...
class KafkaEventBridge:
    """Bridges therapy events to Kafka via the local EventBus."""
    ...
    def __init__(
        ...
        self._started = False
        logger.info("therapy_kafka_bridge_initialized", use_mock=use_mock)
    ...
    async def start(self) -> None:
        ...
        if self._publisher and not self._started:
            ...
            logger.info("therapy_kafka_bridge_started")
    ...
    async def stop(self) -> None:
        ...
        if self._publisher and self._started:
            ...
            self._started = False
            logger.info("therapy_kafka_bridge_stopped")
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        try:
            ...
            if kafka_event:
                ...
                logger.debug(
                    "therapy_event_bridged",
                    event_type=event.event_type.value,
                    kafka_type=kafka_event.event_type,
                )
        except Exception as e:
            logger.error(
                "therapy_kafka_bridge_error",
                event_type=event.event_type.value,
                error=str(e),
            )
    ...
    async def bridge_event(self, event: DomainEvent) -> bool:
        ...
        try:
            ...
            logger.error("therapy_kafka_bridge_error", error=str(e))
            return False
