...
class SymptomExtractor:
    ...
    def _build_symptom_patterns(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "elevated_mood": {
                "patterns": [r"\b(elat|euphor|on\s+top\s+of\s+the\s+world|manic|hyper)\b",
                            r"\b(high\s+energy|invincible|unstoppable)\b"],
                "type": SymptomType.EMOTIONAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "mood_disorders",
            },
            "grandiosity": {
                "patterns": [r"\b(grandiose|special\s+powers|chosen|superior|genius)\b",
                            r"\b(inflated|above\s+everyone)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "mood_disorders",
            },
            "racing_thoughts": {
                "patterns": [r"\b(racing\s+thoughts|thoughts\s+racing|mind\s+won't\s+stop)\b",
                            r"\b(can't\s+slow\s+down|mind\s+racing)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "mood_disorders",
            },
            "impulsivity": {
                "patterns": [r"\b(impulsiv|act\s+without\s+thinking|reckless|rash\s+decisions)\b",
                            r"\b(can't\s+control|spur\s+of\s+the\s+moment)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "behavioral_disorders",
            },
            "compulsive_behavior": {
                "patterns": [r"\b(compuls|ritual|have\s+to\s+check|must\s+do|washing\s+hands)\b",
                            r"\b(counting|repeat|can't\s+stop\s+doing)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "ocd_disorders",
            },
            "hyperactivity": {
                "patterns": [r"\b(hyperactiv|fidget|can't\s+sit\s+still|restless|squirm)\b",
                            r"\b(always\s+moving|bouncing|pacing)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MILD,
                "dsm_category": "adhd",
            },
            "inattention": {
                "patterns": [r"\b(inattenti|easily\s+distracted|zoning\s+out|daydream)\b",
                            r"\b(lose\s+track|careless\s+mistakes|don't\s+listen)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MILD,
                "dsm_category": "adhd",
            },
            "body_image_distortion": {
                "patterns": [r"\b(fat|ugly|body\s+image|hate\s+my\s+body|too\s+heavy)\b",
                            r"\b(look\s+disgusting|need\s+to\s+be\s+thin)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "eating_disorders",
            },
            "binge_eating": {
                "patterns": [r"\b(binge|eating\s+out\s+of\s+control|can't\s+stop\s+eating)\b",
                            r"\b(eat\s+everything|gorging|stuffing)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "eating_disorders",
            },
            "purging": {
                "patterns": [r"\b(purg|throw\s+up|vomit|laxativ|make\s+myself\s+sick)\b",
                            r"\b(get\s+rid\s+of\s+food)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.SEVERE,
                "dsm_category": "eating_disorders",
            },
            "food_restriction": {
                "patterns": [r"\b(restrict|starv|skip\s+meals|not\s+allow\s+myself)\b",
                            r"\b(fasting|refusing\s+to\s+eat|calorie\s+counting)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "eating_disorders",
            },
            "substance_use": {
                "patterns": [r"\b(drink\w*\s+too\s+much|using\s+drugs|addicted|dependent)\b",
                            r"\b(can't\s+stop\s+drinking|substance|alcohol\s+problem)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "substance_use",
            },
            "craving": {
                "patterns": [r"\b(crav|need\s+a\s+drink|urge\s+to\s+use|desperate\s+for)\b",
                            r"\b(can't\s+resist|obsessed\s+with\s+using)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "substance_use",
            },
            "withdrawal": {
                "patterns": [r"\b(withdraw\w*\s+symptoms|shak\w*|sweat\w*\s+without)\b",
                            r"\b(nausea|detox|coming\s+down)\b"],
                "type": SymptomType.SOMATIC,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "substance_use",
            },
            "emotional_instability": {
                "patterns": [r"\b(mood\s+swings|emotions\s+all\s+over|up\s+and\s+down)\b",
                            r"\b(emotional\s+roller.?coaster|can't\s+control\s+emotions)\b"],
                "type": SymptomType.EMOTIONAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "personality_disorders",
            },
            "abandonment_fear": {
                "patterns": [r"\b(afraid.+leav|terrified.+abandon|don't\s+leave\s+me)\b",
                            r"\b(everyone\s+leaves|fear.+alone|clingy)\b"],
                "type": SymptomType.EMOTIONAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "personality_disorders",
            },
            "identity_disturbance": {
                "patterns": [r"\b(don't\s+know\s+who\s+I\s+am|no\s+sense\s+of\s+self)\b",
                            r"\b(identity\s+crisis|empty\s+inside|feel\s+like\s+nothing)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "personality_disorders",
            },
            "hopelessness": {
                "patterns": [r"\b(hopeless|no\s+hope|nothing\s+will\s+get\s+better|pointless)\b",
                            r"\b(what's\s+the\s+point|never\s+get\s+better|no\s+future)\b"],
                "type": SymptomType.COGNITIVE,
                "base_severity": SeverityLevel.MODERATELY_SEVERE,
                "dsm_category": "mood_disorders",
            },
            "avoidance": {
                "patterns": [r"\b(avoid\w*|stay\s+away|can't\s+face|don't\s+go)\b",
                            r"\b(refuse\s+to|won't\s+go\s+near|steer\s+clear)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "anxiety_disorders",
            },
            "self_harm": {
                "patterns": [r"\b(hurt\w*\s+myself|self.?harm|cutting|burning\s+myself)\b",
                            r"\b(scratch\w*\s+myself|hit\w*\s+myself)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.SEVERE,
                "dsm_category": "risk_behaviors",
            },
            "risk_taking": {
                "patterns": [r"\b(reckless|risky\s+behavio|dangerous\s+things|gambling)\b",
                            r"\b(spending\s+spree|unsafe\s+sex|speeding)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "behavioral_disorders",
            },
            "hypervigilance": {
                "patterns": [r"\b(on\s+guard|hypervigilant|always\s+watching|startle)\b",
                            r"\b(jump\s+at|can't\s+relax|constantly\s+alert)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "trauma_disorders",
            },
            "tolerance": {
                "patterns": [r"\b(need\s+more|doesn't\s+work\s+anymore|increased\s+dose)\b",
                            r"\b(tolerance|takes\s+more|used\s+to\s+be\s+enough)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "substance_use",
            },
            "decreased_need_for_sleep": {
                "patterns": [r"\b(don't\s+need\s+sleep|only\s+slept\s+\d|few\s+hours\s+sleep)\b",
                            r"\b(wide\s+awake|full\s+of\s+energy\s+without\s+sleep)\b"],
                "type": SymptomType.SOMATIC,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "mood_disorders",
            },
            "pressured_speech": {
                "patterns": [r"\b(talking\s+fast|can't\s+stop\s+talking|rapid\s+speech)\b",
                            r"\b(people\s+can't\s+follow|jumping\s+from\s+topic)\b"],
                "type": SymptomType.BEHAVIORAL,
                "base_severity": SeverityLevel.MODERATE,
                "dsm_category": "mood_disorders",
            },
            "restlessness": {
                "patterns": [r"\b(restless|can't\s+sit|pacing|keyed\s+up)\b",
                            r"\b(on\s+edge|agitated|wound\s+up)\b"],
                "type": SymptomType.SOMATIC,
                "base_severity": SeverityLevel.MILD,
                "dsm_category": "anxiety_disorders",
            },
