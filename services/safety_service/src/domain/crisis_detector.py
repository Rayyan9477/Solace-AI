...
class DetectionResult(BaseModel):
    ...
    risk_score: Decimal = Field(default=Decimal("0.0"), ge=0, le=1, description="Overall risk score")
    ...
    confidence: Decimal = Field(default=Decimal("0.0"), ge=0, le=1, description="Overall confidence")
...
class CrisisDetectorSettings(BaseSettings):
    ...
    keyword_weight: Decimal = Field(default=Decimal("0.4"), description="Weight for keyword detection")
    sentiment_weight: Decimal = Field(default=Decimal("0.2"), description="Weight for sentiment analysis")
    pattern_weight: Decimal = Field(default=Decimal("0.25"), description="Weight for pattern matching")
    ...
    elevated_threshold: Decimal = Field(default=Decimal("0.5"), description="Threshold for ELEVATED risk")
    high_threshold: Decimal = Field(default=Decimal("0.7"), description="Threshold for HIGH risk")
    critical_threshold: Decimal = Field(default=Decimal("0.9"), description="Threshold for CRITICAL risk")
...
class KeywordSet:
    ...
    high_keywords: list[str] = field(default_factory=lambda: [
        "self-harm", "hurt myself", "cutting", "overdose", "don't want to live",
        "hopeless", "no hope", "give up", "worthless", "burden",
    ])
    elevated_keywords: list[str] = field(default_factory=lambda: [
        "depressed", "anxious", "scared", "overwhelmed", "can't cope",
        "falling apart", "breaking down", "losing control", "panic",
    ])
    low_keywords: list[str] = field(default_factory=lambda: [
        "sad", "stressed", "worried", "upset", "frustrated", "tired", "exhausted",
    ])
...
class Layer1InputGate:
    ...
    def _compile_patterns(self) -> dict[str, re.Pattern]:
        ...
        return {
            "suicidal_ideation": re.compile(r"\b(want|going|planning|thinking)\s+(to\s+)?(die|kill|end|suicide)\b", re.I),
            "self_harm": re.compile(r"\b(cut|cutting|burn|burning|hurt|hurting)\s+(myself|my\s+\w+)\b", re.I),
            "hopelessness": re.compile(r"\b(no\s+(hope|point|reason)|never\s+get\s+better|always\s+be\s+this\s+way)\b", re.I),
            "plan_indicators": re.compile(r"\b(have\s+a\s+plan|know\s+how|figured\s+out|pills|gun|rope|bridge)\b", re.I),
            "farewell": re.compile(r"\b(goodbye|farewell|sorry\s+for\s+everything|tell\s+\w+\s+i\s+love)\b", re.I),
            "timeframe": re.compile(r"\b(tonight|today|tomorrow|soon|this\s+week)\s+.*(end|die|gone|over)\b", re.I),
        }

    def detect(self, content: str, user_risk_history: dict[str, Any] | None = None) -> DetectionResult:
        ...
        history_score = self._check_risk_history(user_risk_history, risk_factors) if user_risk_history else Decimal("0")
        max_signal = max(keyword_score, pattern_score)
        base_score = max_signal * Decimal("0.85") + history_score * Decimal("0.15")
        boosted_score = base_score + (pattern_score * Decimal("0.1") if pattern_score > Decimal("0") and keyword_score > Decimal("0") else Decimal("0"))
        total_score = min(boosted_score, Decimal("1.0"))
        detection_time_ms = int((time.perf_counter() - start_time) * 1000)
        crisis_level = CrisisLevel.from_score(total_score)
    ...
    def _detect_keywords(self, content: str, risk_factors: list[RiskFactor], triggers: list[str]) -> Decimal:
        """Detect crisis keywords in content."""
        score = Decimal("0")
        for kw in self._keywords.critical_keywords:
            if kw in content:
                score = max(score, Decimal("0.95"))
                risk_factors.append(RiskFactor(factor_type="critical_keyword", severity=Decimal("0.95"),
                    evidence=f"Critical keyword detected: '{kw}'", confidence=Decimal("0.9"), detection_layer=1))
                triggers.append(f"CRITICAL_KEYWORD:{kw}")
        for kw in self._keywords.high_keywords:
            if kw in content:
                score = max(score, Decimal("0.75"))
                risk_factors.append(RiskFactor(factor_type="high_keyword", severity=Decimal("0.75"),
                    evidence=f"High-risk keyword detected: '{kw}'", confidence=Decimal("0.85"), detection_layer=1))
                triggers.append(f"HIGH_KEYWORD:{kw}")
        for kw in self._keywords.elevated_keywords:
            if kw in content:
                score = max(score, Decimal("0.5"))
                risk_factors.append(RiskFactor(factor_type="elevated_keyword", severity=Decimal("0.5"),
                    evidence=f"Elevated keyword detected: '{kw}'", confidence=Decimal("0.7"), detection_layer=1))
        for kw in self._keywords.low_keywords:
            if kw in content:
                score = max(score, Decimal("0.25"))
        return score

    def _detect_patterns(self, content: str, risk_factors: list[RiskFactor], triggers: list[str]) -> Decimal:
        """Detect crisis patterns using regex."""
        max_score = Decimal("0")
        pattern_scores = {"suicidal_ideation": Decimal("0.95"), "self_harm": Decimal("0.8"),
            "hopelessness": Decimal("0.6"), "plan_indicators": Decimal("0.9"),
            "farewell": Decimal("0.85"), "timeframe": Decimal("0.9")}
        for pattern_name, pattern in self._patterns.items():
            if pattern.search(content):
                score = pattern_scores.get(pattern_name, Decimal("0.5"))
                max_score = max(max_score, score)
                risk_factors.append(RiskFactor(factor_type=f"pattern_{pattern_name}", severity=score,
                    evidence=f"Pattern '{pattern_name}' detected", confidence=Decimal("0.8"), detection_layer=1))
                triggers.append(f"PATTERN:{pattern_name}")
        return max_score

    def _check_risk_history(self, history: dict[str, Any], risk_factors: list[RiskFactor]) -> Decimal:
        """Check user's risk history for elevated baseline."""
        score = Decimal("0")
        if history.get("previous_crisis_events", 0) > 0:
            score = Decimal("0.3")
            risk_factors.append(RiskFactor(factor_type="risk_history", severity=Decimal("0.3"),
                evidence="Previous crisis events in history", confidence=Decimal("0.7"), detection_layer=1))
    ...
    def _get_recommended_action(self, level: CrisisLevel) -> str:
        ...
        actions = {CrisisLevel.NONE: "continue", CrisisLevel.LOW: "monitor",
            CrisisLevel.ELEVATED: "assess", CrisisLevel.HIGH: "intervene", CrisisLevel.CRITICAL: "escalate_immediately"}
...
class Layer2ProcessingGuard:
    ...
    def validate_context(self, content: str, context: dict[str, Any], layer1_result: DetectionResult) -> DetectionResult:
        ...
        if active_technique and layer1_result.crisis_level in self._contraindicated_techniques:
            ...
            if active_technique in contraindicated:
                ...
                risk_factors.append(RiskFactor(factor_type="contraindication", severity=Decimal("0.7"),
                    evidence=f"Contraindicated technique '{active_technique}' for {layer1_result.crisis_level.value}",
                    confidence=Decimal("0.95"), detection_layer=2))
                triggers.append(f"CONTRAINDICATION:{active_technique}")
                layers_triggered.append(2)
        severity_mismatch = self._check_severity_appropriateness(context, layer1_result.crisis_level)
        ...
        detection_time_ms = layer1_result.detection_time_ms + int((time.perf_counter() - start_time) * 1000)
        crisis_level = CrisisLevel.from_score(risk_score)
        return DetectionResult(
            crisis_detected=crisis_level != CrisisLevel.NONE,
            crisis_level=crisis_level,
    ...
    def _check_severity_appropriateness(self, context: dict[str, Any], crisis_level: CrisisLevel) -> RiskFactor | None:
        """Check if intervention severity matches crisis level."""
        intervention_intensity = context.get("intervention_intensity", "normal")
        if crisis_level in (CrisisLevel.HIGH, CrisisLevel.CRITICAL) and intervention_intensity == "high":
            return RiskFactor(factor_type="severity_mismatch", severity=Decimal("0.4"),
                evidence="High intensity intervention inappropriate for crisis state",
                confidence=Decimal("0.8"), detection_layer=2)
        return None
...
class Layer3OutputFilter:
    ...
    def filter_output(self, response: str, crisis_level: CrisisLevel) -> tuple[str, list[str], bool]:
        ...
        for phrase in self._unsafe_phrases:
            if phrase.lower() in filtered.lower():
                ...
                filtered = re.sub(re.escape(phrase), "[supportive message]", filtered, flags=re.IGNORECASE)
        if crisis_level in self._required_elements:
            ...
            if missing:
                modifications.append(f"Response missing required elements for {crisis_level.value}: {missing}")
...
class Layer4ContinuousMonitor:
    ...
    def analyze_trajectory(self, conversation_history: list[str], current_level: CrisisLevel) -> dict[str, Any]:
        """Analyze session trajectory for deterioration patterns."""
        if len(conversation_history) < 2:
            return {"trend": "insufficient_data", "deteriorating": False, "risk_delta": Decimal("0")}
        recent_negative_count = sum(1 for msg in conversation_history[-5:] if self._is_negative(msg))
        total_messages = min(len(conversation_history), 5)
        negative_ratio = Decimal(str(recent_negative_count / total_messages)) if total_messages > 0 else Decimal("0")
        deteriorating = negative_ratio > Decimal("0.6") and current_level in (CrisisLevel.ELEVATED, CrisisLevel.HIGH)
        trend = "deteriorating" if deteriorating else "stable" if negative_ratio < Decimal("0.4") else "concerning"
        return {"trend": trend, "deteriorating": deteriorating, "risk_delta": negative_ratio - Decimal("0.3"),
            "negative_ratio": negative_ratio, "recent_messages_analyzed": total_messages}

    def _is_negative(self, message: str) -> bool:
        """Check if message has negative sentiment indicators."""
        negative_indicators = ["worse", "bad", "terrible", "hopeless", "can't", "won't", "never", "hate", "alone"]
...
class CrisisDetector:
    ...
    def __init__(self, settings: CrisisDetectorSettings | None = None) -> None:
        ...
        logger.info("crisis_detector_initialized", layers_enabled={
            "layer1": self._settings.enable_layer_1, "layer2": self._settings.enable_layer_2,
            "layer3": self._settings.enable_layer_3, "layer4": self._settings.enable_layer_4})

    async def detect(self, content: str, context: dict[str, Any] | None = None,
                     conversation_history: list[str] | None = None,
                     user_risk_history: dict[str, Any] | None = None) -> DetectionResult:
        """Perform multi-layer crisis detection."""
        context = context or {}
        result = self._layer1.detect(content, user_risk_history) if self._settings.enable_layer_1 else DetectionResult()
        ...
        if self._settings.enable_layer_4 and conversation_history:
            ...
            if trajectory.get("deteriorating"):
                ...
                result.crisis_level = CrisisLevel.from_score(result.risk_score)
        ...
        logger.info("crisis_detection_complete", crisis_level=result.crisis_level.value,
            risk_score=float(result.risk_score), layers_triggered=result.detection_layers_triggered)
        return result
