"""Unit tests for DeepSeek client module."""
from __future__ import annotations
import pytest
from solace_ml.deepseek import (
    DeepSeekSettings, DeepSeekClient, create_deepseek_client,
)
...
class TestDeepSeekSettings:
    """Tests for DeepSeekSettings model."""
    ...
    def test_custom_settings(self):
        settings = DeepSeekSettings(model="deepseek-coder", max_tokens=8192)
        assert settings.model == "deepseek-coder"
        assert settings.max_tokens == 8192

    def test_penalty_settings(self):
        settings = DeepSeekSettings(frequency_penalty=0.5, presence_penalty=0.3)
        assert settings.frequency_penalty == 0.5
        assert settings.presence_penalty == 0.3

    def test_response_format(self):
        settings = DeepSeekSettings(response_format="json_object")
...
class TestDeepSeekClient:
    ...
    def test_client_with_settings(self):
        settings = DeepSeekSettings(model="deepseek-coder")
...
class TestFactoryFunction:
    ...
    def test_create_deepseek_client_with_settings(self):
        settings = DeepSeekSettings(model="deepseek-coder")
