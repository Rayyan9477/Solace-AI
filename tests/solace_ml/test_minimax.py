"""Unit tests for Minimax client module."""
from __future__ import annotations
import pytest
from solace_ml.minimax import (
    MinimaxSettings, MinimaxClient, create_minimax_client,
)
...
class TestMinimaxSettings:
    """Tests for MinimaxSettings model."""

    def test_default_settings(self):
        settings = MinimaxSettings()
        assert settings.model == "MiniMax-Text-01"
        assert settings.max_tokens == 4096
        assert settings.mask_sensitive_info is True

    def test_custom_settings(self):
        settings = MinimaxSettings(model="abab6.5-chat", max_tokens=8192)
        assert settings.model == "abab6.5-chat"
        assert settings.max_tokens == 8192

    def test_group_id_setting(self):
        settings = MinimaxSettings(group_id="group123")
        assert settings.group_id == "group123"

    def test_mask_sensitive_setting(self):
        settings = MinimaxSettings(mask_sensitive_info=False)
        assert settings.mask_sensitive_info is False
...
class TestMinimaxClient:
    ...
    def test_client_creation(self, client):
        assert client.provider == LLMProvider.MINIMAX
        assert client.model == "MiniMax-Text-01"

    def test_client_with_settings(self):
        settings = MinimaxSettings(model="abab6.5-chat")
        client = MinimaxClient(settings)
        assert client.model == "abab6.5-chat"
    ...
    def test_build_payload_with_mask_sensitive(self):
        settings = MinimaxSettings(mask_sensitive_info=True)
        client = MinimaxClient(settings)
        messages = [Message(role=MessageRole.USER, content="Hello")]
        payload = client._build_payload(messages, None, None)
        assert payload["mask_sensitive_info"] is True
...
class TestFactoryFunction:
    ...
    def test_create_minimax_client_with_settings(self):
        settings = MinimaxSettings(model="abab6.5-chat")
        client = create_minimax_client(settings)
        assert client.model == "abab6.5-chat"
