...
model = AutoModelForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=len(intents['intents'])).to(device)
...
missing_files = [f for f in required_files if not os.path.exists(os.path.join('fine_tuned_model', f))]
if missing_files:
