...
class WeaviateClient:
        ...
        try:
            def _create() -> bool:
                ...
                client.collections.create(
                    name=config.name, description=config.description, properties=properties,
                    vectorizer_config=Configure.Vectorizer.none() if not config.vectorizer else None,
                    ...
                    multi_tenancy_config=Configure.multi_tenancy(enabled=config.multi_tenancy_enabled),
                )
                return True
    ...
    def _build_property(self, prop: PropertyConfig) -> Property:
        ...
        return Property(
            name=prop.name, data_type=dtype, description=prop.description,
            skip_vectorization=prop.skip_vectorization,
            index_filterable=prop.index_filterable, index_searchable=prop.index_searchable,
        )
    ...
    async def insert(self, collection: str, properties: dict[str, Any],
                     vector: list[float] | None = None, uuid: UUID | None = None) -> UUID:
    ...
    async def insert_batch(self, collection: str, objects: list[dict[str, Any]],
                           vectors: list[list[float]] | None = None) -> list[UUID]:
    ...
    async def get_by_id(self, collection: str, uuid: UUID) -> SearchResult | None:
        ...
        def _get() -> SearchResult | None:
            ...
            if not obj:
                return None
            return SearchResult(uuid=obj.uuid, properties=obj.properties, vector=obj.vector)
    ...
    async def vector_search(self, collection: str, vector: list[float], limit: int = 10,
        ...
        def _search() -> list[SearchResult]:
            ...
            query = coll.query.near_vector(
                near_vector=vector, limit=limit, include_vector=return_vector,
                return_metadata=MetadataQuery(distance=True, certainty=True),
            )
            return [
                SearchResult(uuid=obj.uuid, properties=obj.properties,
                            vector=obj.vector if return_vector else None,
                            distance=obj.metadata.distance, certainty=obj.metadata.certainty)
                for obj in query.objects
            ]
        return await loop.run_in_executor(None, _search)

    async def hybrid_search(self, collection: str, query: str, vector: list[float] | None = None,
                            limit: int = 10, alpha: float = 0.5) -> list[SearchResult]:
        ...
        def _search() -> list[SearchResult]:
            ...
            result = coll.query.hybrid(
                query=query, vector=vector, limit=limit, alpha=alpha,
                return_metadata=MetadataQuery(score=True),
            )
            return [
                SearchResult(uuid=obj.uuid, properties=obj.properties, score=obj.metadata.score)
                for obj in result.objects
            ]
        return await loop.run_in_executor(None, _search)

    async def update(self, collection: str, uuid: UUID, properties: dict[str, Any],
...
async def create_weaviate_client(settings: WeaviateSettings | None = None) -> WeaviateClient:
