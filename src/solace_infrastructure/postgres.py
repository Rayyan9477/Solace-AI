...
class PostgresClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info("postgres_pool_connected", host=self._settings.host,
                       database=self._settings.database, pool_size=self._settings.max_pool_size)
        except asyncpg.PostgresError as e:
            raise DatabaseError(f"Failed to connect to PostgreSQL: {e}", operation="connect", cause=e)

    async def _init_connection(self, conn: asyncpg.Connection) -> None:
        """Initialize each connection with type codecs."""
        await conn.set_type_codec("json", encoder=_json_encoder, decoder=_json_decoder,
                                   schema="pg_catalog")
        await conn.set_type_codec("jsonb", encoder=_json_encoder, decoder=_json_decoder,
                                   schema="pg_catalog")
    ...
    def _ensure_connected(self) -> asyncpg.Pool:
        if not self._pool:
            raise DatabaseError("PostgreSQL client not connected", operation="check_connection")
        return self._pool
    ...
    @asynccontextmanager
    async def transaction(self, isolation: IsolationLevel = IsolationLevel.READ_COMMITTED
    ...
    async def execute(self, query: str, *args: Any, timeout: float | None = None) -> str:
        ...
        async with self.acquire() as conn:
            try:
                ...
                logger.debug("postgres_execute", query=_truncate_query(query), elapsed_ms=elapsed)
                return result
            except asyncpg.PostgresError as e:
                raise DatabaseError(f"Query execution failed: {e}", operation="execute", cause=e)

    async def fetch(self, query: str, *args: Any, timeout: float | None = None) -> QueryResult:
        ...
        async with self.acquire() as conn:
            try:
                ...
                logger.debug("postgres_fetch", query=_truncate_query(query),
                            row_count=result.row_count, elapsed_ms=elapsed)
                return result
            except asyncpg.PostgresError as e:
                raise DatabaseError(f"Query fetch failed: {e}", operation="fetch", cause=e)

    async def fetch_one(self, query: str, *args: Any, timeout: float | None = None
                       ) -> dict[str, Any] | None:
        ...
        async with self.acquire() as conn:
            try:
                ...
                raise DatabaseError(f"Query fetch_one failed: {e}", operation="fetch_one", cause=e)

    async def fetch_val(self, query: str, *args: Any, column: int = 0,
        ...
        async with self.acquire() as conn:
            try:
                ...
                raise DatabaseError(f"Query fetch_val failed: {e}", operation="fetch_val", cause=e)
    ...
    async def execute_many(self, query: str, args_list: list[tuple[Any, ...]],
        ...
        async with self.acquire() as conn:
            try:
                ...
                logger.debug("postgres_execute_many", query=_truncate_query(query),
                            batch_size=len(args_list))
            except asyncpg.PostgresError as e:
                raise DatabaseError(f"Batch execution failed: {e}", operation="execute_many", cause=e)

    async def copy_records(self, table_name: str, records: list[tuple[Any, ...]],
                           columns: list[str], timeout: float | None = None) -> str:
        ...
        async with self.acquire() as conn:
            try:
                ...
                logger.info("postgres_copy_records", table=table_name, record_count=len(records))
                return result
            except asyncpg.PostgresError as e:
                raise DatabaseError(f"Copy records failed: {e}", operation="copy_records", cause=e)
...
class PostgresRepository:
    ...
    def __init__(self, client: PostgresClient, table_name: str, schema: str = "public") -> None:
        self._client = client
    ...
    async def update(self, entity_id: UUID | str, data: dict[str, Any]) -> dict[str, Any] | None:
        """Update entity and return updated record."""
        data["updated_at"] = datetime.now(timezone.utc)
        set_clause = ", ".join(f"{k} = ${i+2}" for i, k in enumerate(data.keys()))
        query = f"UPDATE {self.qualified_table} SET {set_clause} WHERE id = $1 RETURNING *"
    ...
    async def soft_delete(self, entity_id: UUID | str) -> bool:
        ...
        result = await self._client.execute(query, entity_id, datetime.now(timezone.utc))
...
async def create_postgres_client(settings: PostgresSettings | None = None) -> PostgresClient:
