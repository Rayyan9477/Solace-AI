...
class AuthSettings(BaseSettings):
    ...
    secret_key: SecretStr = Field(default=SecretStr("change-me-in-production-32bytes!"))
...
class PasswordHasher:
    ...
    def hash_password(cls, password: str) -> str:
        ...
        dk = hashlib.pbkdf2_hmac(cls._hash_name, password.encode(), salt, cls._iterations)
    ...
    def verify_password(cls, password: str, stored_hash: str) -> bool:
        try:
            ...
            dk = hashlib.pbkdf2_hmac(hash_name, password.encode(), salt, int(iterations))
...
class JWTManager:
    ...
    def create_access_token(self, user_id: str, roles: list[str] | None = None,
        ...
        payload = TokenPayload(
            sub=user_id, type=TokenType.ACCESS, iat=now, exp=exp,
            iss=self._settings.issuer, aud=self._settings.audience,
    ...
    def create_refresh_token(self, user_id: str, session_id: str | None = None) -> str:
        ...
        payload = TokenPayload(
            sub=user_id, type=TokenType.REFRESH, iat=now, exp=exp,
            iss=self._settings.issuer, aud=self._settings.audience, session_id=session_id,
        )
        return self._encode(payload)

    def create_token_pair(self, user_id: str, roles: list[str] | None = None,
                          permissions: list[str] | None = None,
    ...
    def create_api_key(self, user_id: str, name: str, permissions: list[str] | None = None) -> str:
        now = int(time.time())
        exp = now + (self._settings.api_key_expire_days * 86400)
        payload = TokenPayload(
            sub=user_id, type=TokenType.API_KEY, iat=now, exp=exp,
            iss=self._settings.issuer, aud=self._settings.audience,
            permissions=permissions or [], metadata={"api_key_name": name},
        )
        return self._encode(payload)

    def create_service_token(self, service_name: str, permissions: list[str] | None = None,
                             expire_minutes: int = 60) -> str:
        now = int(time.time())
        exp = now + (expire_minutes * 60)
        payload = TokenPayload(
            sub=f"service:{service_name}", type=TokenType.SERVICE, iat=now, exp=exp,
            iss=self._settings.issuer, aud=self._settings.audience,
            permissions=permissions or [], metadata={"service_name": service_name},
        )
        return self._encode(payload)

    def decode_token(self, token: str, expected_type: TokenType | None = None) -> AuthenticationResult:
        try:
            decoded = jwt.decode(
                token, self._secret, algorithms=[self._settings.algorithm],
            ...
            if expected_type and payload.type != expected_type:
                return AuthenticationResult.fail("INVALID_TOKEN_TYPE",
            ...
            return AuthenticationResult.fail("INVALID_AUDIENCE", "Invalid token audience")
        except jwt.DecodeError as e:
            return AuthenticationResult.fail("DECODE_ERROR", f"Failed to decode token: {e}")
    ...
    def refresh_access_token(self, refresh_token: str, roles: list[str] | None = None,
                             permissions: list[str] | None = None) -> tuple[str, AuthenticationResult]:
    ...
    def _encode(self, payload: TokenPayload) -> str:
        return jwt.encode(payload.model_dump(), self._secret, algorithm=self._settings.algorithm)
...
class APIKeyGenerator:
    ...
    def validate_format(cls, api_key: str) -> bool:
        return api_key.startswith(cls._prefix) and len(api_key) == len(cls._prefix) + cls._key_length * 2
...
class SessionManager:
    ...
    def create_session(self, user_id: str, metadata: dict[str, Any] | None = None) -> str:
        ...
        self._active_sessions[session_id] = {
            ...
            "last_activity": datetime.now(timezone.utc), "metadata": metadata or {},
        }
    ...
    def invalidate_user_sessions(self, user_id: str) -> int:
        to_remove = [sid for sid, data in self._active_sessions.items() if data["user_id"] == user_id]
        for sid in to_remove:
            del self._active_sessions[sid]
        if to_remove:
            logger.info("user_sessions_invalidated", user_id=user_id, count=len(to_remove))
        return len(to_remove)

    def get_user_sessions(self, user_id: str) -> list[str]:
        return [sid for sid, data in self._active_sessions.items() if data["user_id"] == user_id]
