"""Solace-AI Minimax Client - Minimax adapter with streaming support."""
...
class MinimaxSettings(LLMSettings):
    """Minimax-specific settings."""
    api_key: SecretStr = Field(default=SecretStr(""))
    model: str = Field(default="MiniMax-Text-01")
    max_tokens: int = Field(default=4096, ge=1, le=1000000)
    group_id: str = Field(default="", description="Minimax group ID")
    mask_sensitive_info: bool = Field(default=True)
    model_config = SettingsConfigDict(env_prefix="MINIMAX_", env_file=".env", extra="ignore")
...
class MinimaxClient(LLMClient):
    ...
    def __init__(self, settings: MinimaxSettings | None = None) -> None:
        settings = settings or MinimaxSettings()
        super().__init__(settings, LLMProvider.MINIMAX)
        self._settings: MinimaxSettings = settings
    ...
    async def complete(self, messages: list[Message], *, tools: list[ToolDefinition] | None = None,
        ...
        try:
            response = await self._http.post(
                MINIMAX_API_URL, json=payload, headers=self._build_headers()
            )
    ...
    async def stream(self, messages: list[Message], *, tools: list[ToolDefinition] | None = None,
        ...
        """Stream completion using Minimax."""
        payload = self._build_payload(messages, tools, system_prompt, stream=True, **kwargs)
        accumulated_tool_calls: dict[int, dict[str, Any]] = {}
        try:
            async with self._http.stream(
                "POST", MINIMAX_API_URL, json=payload, headers=self._build_headers()
            ) as response:
                self._check_response(response)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning("health_check_failed", provider="minimax", error=str(e))
            return False
    ...
    def _build_payload(self, messages: list[Message], tools: list[ToolDefinition] | None,
        ...
        if self._settings.mask_sensitive_info:
            payload["mask_sensitive_info"] = self._settings.mask_sensitive_info
        if tools:
    ...
    def _convert_tool(self, tool: ToolDefinition) -> dict[str, Any]:
        """Convert tool definition to Minimax format."""
        return {
            "type": "function",
            "function": {
                "name": tool.name,
    ...
    def _parse_response(self, data: dict[str, Any], latency: float, request_id: str) -> LLMResponse:
        ...
        if base_resp.get("status_code") != 0:
            raise LLMError(f"API error: {base_resp.get('status_msg', 'Unknown error')}",
                          provider=self._provider, error_type="api_error")
        choices = data.get("choices", [{}])
        if not choices:
            return LLMResponse(content="", provider=self._provider, latency_ms=latency, request_id=request_id)
        choice = choices[0]
        ...
        return LLMResponse(
            ...
            provider=self._provider, latency_ms=latency, request_id=data.get("id", request_id)
        )
    ...
    def _map_finish_reason(self, reason: str | None) -> FinishReason:
        """Map Minimax finish reason to standard enum."""
        mapping = {
...
def create_minimax_client(settings: MinimaxSettings | None = None) -> MinimaxClient:
...
async def quick_complete(prompt: str, *, model: str | None = None, system: str | None = None) -> str:
    ...
    try:
        settings = MinimaxSettings(model=model) if model else None
        if settings:
