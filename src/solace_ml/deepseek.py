"""Solace-AI DeepSeek Client - DeepSeek adapter with OpenAI-compatible API."""
...
class DeepSeekSettings(LLMSettings):
    ...
    response_format: str | None = Field(default=None, description="Response format (json_object)")
    model_config = SettingsConfigDict(env_prefix="DEEPSEEK_", env_file=".env", extra="ignore")


class DeepSeekClient(LLMClient):
    """DeepSeek API client with streaming (OpenAI-compatible)."""

    def __init__(self, settings: DeepSeekSettings | None = None) -> None:
        settings = settings or DeepSeekSettings()
        super().__init__(settings, LLMProvider.DEEPSEEK)
        self._settings: DeepSeekSettings = settings
    ...
    async def complete(self, messages: list[Message], *, tools: list[ToolDefinition] | None = None,
        ...
        try:
            response = await self._http.post(
                DEEPSEEK_API_URL, json=payload, headers=self._build_headers()
            )
    ...
    async def stream(self, messages: list[Message], *, tools: list[ToolDefinition] | None = None,
        ...
        try:
            async with self._http.stream(
                "POST", DEEPSEEK_API_URL, json=payload, headers=self._build_headers()
            ) as response:
                self._check_response(response)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning("health_check_failed", provider="deepseek", error=str(e))
            return False
    ...
    def _map_finish_reason(self, reason: str | None) -> FinishReason:
        """Map DeepSeek finish reason to standard enum."""
        mapping = {
...
def create_deepseek_client(settings: DeepSeekSettings | None = None) -> DeepSeekClient:
...
async def quick_complete(prompt: str, *, model: str | None = None, system: str | None = None) -> str:
    ...
    try:
        settings = DeepSeekSettings(model=model) if model else None
        if settings:
